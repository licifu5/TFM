...
#CLASE QUE RESUME TODOS LOS MÉTODOS ESTUDIADOS EN LA LITERATURA

#************************PARTE DEL MODELO RESNET 50***********************************
#metodo que obtiene un vector de caracteristicas a partir de una imagen y el modelo
...
#metodo que compara dos vectores de caracteristicas
def findDifference(f1, f2):
    return np.linalg.norm(f1-f2)
#creacion del modelo resnet
model = ResNet50(weights='imagenet')
#***************************************************************************************************************************************************************

#FUNCIONES DE PREPROCESADO + comparativa de vectores con modelo
...
#FUNCION QUE APLICA LPIPS PARA DETERMINAR LA SIMILITUD ENTRE DOS IMAGENES
def normalize_negative_one(img):
    normalized_input = (img - np.amin(img)) / (np.amax(img) - np.amin(img))
    return 2*normalized_input - 1

def lpips_method(img_path, img_path2):
    img = cv2.imread(img_path)
    img2 = cv2.imread(img_path2)
    ...
    image2=normalize_negative_one(image2)

    convert_tensor = transforms.ToTensor()
    image= convert_tensor(image).float()
    image2= convert_tensor(image2).float()
